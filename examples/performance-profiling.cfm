<cfscript>
	...
	systemOutput("STEP 4: Generating reports from .exl files using coverage-reporter", true);
	systemOutput("This processes the .exl files generated by ConsoleExecutionLog", true);
	internalRequest(template=ExpandPath("/coverage-reporter.cfm"),
		url={
	...
	systemOutput("Look for execution times in the console output above", true);
	systemOutput("", true);
	...
	systemOutput("- Outputs to console (not .exl files)", true);
	systemOutput("- Shows performance data (not line coverage)", true);
	systemOutput("- Optional use of coverageReporter.cfm", true);
	systemOutput("==========================================", true);
</cfscript>