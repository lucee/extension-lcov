component accessors="true" {
	...
	// Based on ExecutionLogParser.cfc with major performance optimizations
	...
	* OPTIMIZED VERSION with pre-aggregation and early filtering
	...
	*/
	public struct function parseExlFile(string exlPath, boolean needSingleFileCoverage = false,
		...
		var coverage = {
			...
			"source": parseFilesOptimized( files, exlPath, allowList, blocklist, arguments.useAstForLinesFound ),
		...
		coverage["coverage"] = parseCoverageOptimized( coverage );

		// Add performance metrics from the optimization processing
		if (structKeyExists(variables, "optimizedPerformanceData")) {
			coverage["parserPerformance"] = variables.optimizedPerformanceData;
		}
	...
	/**
	* PRE-AGGREGATION OPTIMIZATION - Combine identical coverage entries before processing
	*/
	private struct function parseCoverageOptimized( struct coverageData) {
	...
	/**
	* CHARACTER-BASED overlap detection - no line conversion needed
	*/
	private struct function filterOverlappingPositions(struct aggregatedCoverage) {
		// TODO: Implement character-based overlap detection
		// For now, return all positions (no filtering)
		return arguments.aggregatedCoverage;
	}
	...
	*/
	...
	/**
	* OPTIMIZED: File parsing with early validation
	*/
	private struct function parseFilesOptimized(array filesLines, string exlPath,
			array allowList, array blocklist, boolean useAstForLinesFound = true) {
	...
	/**
	* Same as original but with optimized parameters
	*/
